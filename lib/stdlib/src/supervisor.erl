...
-type start_as_child_ret() :: {'ok', pid(), {'child', map()}}
			      | 'ignore'
			      | {'error', startlink_err()}.
        ...
        fun(Id,Child) ->
                case do_start_child(SupName, Child, info_report) of
                    {ok, undefined} when ?is_temporary(Child) ->
                        remove;
                    {ok, Pid} ->
                        {update,Child#child{pid = Pid, alias = Pid}};
		    {ok, Pid, {child, #{alias := Alias}}} ->
			{update, Child#child{pid = Pid, alias = Alias}};
		    {ok, Pid, {child, #{}}} ->
			{update, Child#child{pid = Pid, alias = Pid}};
                    {ok, Pid, _Extra} ->
                        {update,Child#child{pid = Pid, alias = Pid}};
...
handle_call({start_child, EArgs}, _From, State) when ?is_simple(State) ->
	...
	{ok, Pid} ->
	    NState = dyn_store(Pid, Pid, Args, State),
	    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
	{ok, Pid, {child, #{alias := Alias}}} ->
	    NState = dyn_store(Pid, Alias, Args, State),
	    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
	{ok, Pid, {child, #{}}} ->
	    NState = dyn_store(Pid, Pid, Args, State),
	    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
...
handle_call({restart_child, Id}, _From, State) ->
	...
	{ok, Child} when Child#child.pid =:= undefined ->
		...
		{ok, Pid} ->
		    NState = set_pid_and_alias(Pid, Pid, Id, State),
		    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
		{ok, Pid, {child, #{alias := Alias}}} ->
		    NState = set_pid_and_alias(Pid, Alias, Id, State),
		    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
		{ok, Pid, {child, #{}}} ->
		    NState = set_pid_and_alias(Pid, Pid, Id, State),
		    {reply, {ok, Pid}, NState, hibernate_after_action(State)};
...
update_chsp(#child{id=Id}=OldChild, NewDb) ->
    case maps:find(Id, NewDb) of
        {ok,Child} ->
            {ok,NewDb#{Id => Child#child{pid = OldChild#child.pid, alias = OldChild#child.alias}}};
...
handle_start_child(Child, State) ->
	...
	error ->
		...
		{ok, undefined} when ?is_temporary(Child) ->
		    {{ok, undefined}, State};
		{ok, Pid} ->
		    {{ok, Pid}, save_child(Child#child{pid = Pid, alias = Pid}, State)};
		{ok, Pid, {child, #{alias := Alias}}} ->
		    {{ok, Pid}, save_child(Child#child{pid = Pid, alias = Alias}, State)};
		{ok, Pid, {child, #{}}} ->
		    {{ok, Pid}, save_child(Child#child{pid = Pid, alias = Pid}, State)};
		{ok, Pid, Extra} ->
		    {{ok, Pid, Extra}, save_child(Child#child{pid = Pid, alias = Pid}, State)};
...
restart(simple_one_for_one, Child, State0) ->
    ...
    case do_start_child_i(M, F, A) of
        {ok, undefined} ->
            ...
            {ok, State2};
	{ok, Pid} ->
            NState = dyn_store(Pid, Pid, A, State2),
	    {ok, NState};
	{ok, Pid, {child, #{alias := Alias}}} ->
            NState = dyn_store(Pid, Alias, A, State2),
	    {ok, NState};
	{ok, Pid, {child, #{}}} ->
            NState = dyn_store(Pid, Pid, A, State2),
	    {ok, NState};
...
terminate_children(Children, SupName) ->
    Terminate =
        fun(_Id,Child) when ?is_temporary(Child) ->
           ...
           (_Id,Child) ->
                do_terminate(Child, SupName),
                {update,Child#child{pid=undefined, alias=undefined}}
        end,
...
shutdown(#child{pid=Pid, alias=Alias, shutdown=Time} = Child) ->
    Mon = monitor(process, Pid),
    exit(Alias, shutdown, [priority]),
    receive
        {'DOWN', Mon, process, Pid, Reason0} ->
            case unlink_flush(Pid, Reason0) of
                shutdown ->
...
terminate_dynamic_children(State) ->
    ...
    Pids = dyn_fold(
        fun
            (P, Alias, Acc) when is_pid(P), (is_reference(Alias) orelse is_pid(Alias)) ->
                Mon = monitor(process, P),
                case Child#child.shutdown of
                    brutal_kill -> exit(P, kill);
                    _ -> exit(Alias, shutdown)
                end,
                Acc#{{P, Mon} => true};
...
del_child(Id, {Ids,Db}) ->
    case maps:get(Id, Db) of
        ...
        Child ->
            {Ids, Db#{Id=>Child#child{pid=undefined, alias=undefined}}}
    end.
...
split_child(Id, {Ids,Db}) ->
    ...
    {{IdsAfter,DbAfter#{Id=>Ch#child{pid=undefined, alias=undefined}}},{IdsBefore,DbBefore}}.
...
find_dynamic_child(Pid, State) ->
	...
	{ok, Alias} ->
            Child = get_dynamic_child(State),
            {ok, Child#child{pid=Pid, alias=Alias}};
        error ->
            error
    end.
...
set_pid_and_alias(Pid, Alias, Id, #state{children = Children} = State) ->
    State#state{children = set_pid_and_alias(Pid, Alias, Id, Children)};
set_pid_and_alias(Pid, Alias, Id, {Ids, Db}) ->
    NewDb = maps:update_with(Id, fun(Child) -> Child#child{pid=Pid, alias=Alias} end, Db),
    {Ids,NewDb}.
